...
4. **Set up environment variables:**
   ...
   | `PLACEHOLDER_EMOJI`          | `⬛`                | Fallback generic symbol for the `placeholder` condition, used only if `PLACEHOLDER_NOMINAL_EMOJIS` is empty.                                                                                                                                                                                                                                                                                     |
   | `PLACEHOLDER_NOMINAL_EMOJIS` | `🔵,🟠,🟡,🟣,🟤,⚫` | Comma/space-separated set of **distinct, non-semantic** marks for nominal slots in the `placeholder` condition (proportion categories; bar/single-value/trend label & value columns). Each nominal slot shown together gets its own mark, matching the distinguishability of the `semantic` condition while removing meaning. Defaults are equal-weight, hue-coded, and avoid red/green valence. |
   | `PLACEHOLDER_SCALE_EMOJIS`   | `▪️,◾,⬛`          | Comma/space-separated **monotonic neutral ramp** (≥3 marks; low, medium, high) for the ordinal value scale in the `placeholder` condition. Preserves order without topical meaning.                                                                                                                                                                                                              |
